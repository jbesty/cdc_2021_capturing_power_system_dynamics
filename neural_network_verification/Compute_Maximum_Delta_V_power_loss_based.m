function [max_delta_V] = Compute_Maximum_Delta_V_power_loss_based(disturbance_duration, power_output_fraction)
...
% t = 0.999 to fix the instance of the final power delivery.
constraints = [constraints;...
    NN_input(1) == 0.999];
% voltage disturbance duration
constraints = [constraints;...
    NN_input(3) == disturbance_duration];
...
% p.u.
constraints = [constraints; 
    NN_ouput(9) >= 0.8 * power_output_fraction];

options = sdpsettings('solver','gurobi','verbose',0,'savesolveroutput',1);

% objective function - maximise Delta V -> minimise -Delta V
objective_function = -(NN_input(2));
